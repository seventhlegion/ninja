import { dehydrate, QueryClient, QueryKey, useQuery, UseQueryResult } from '@tanstack/react-query';
...
import { ninjaMapProps } from '../../api/type';
import Container from '../../components/Container/Container';

export const getServerSideProps: GetServerSideProps = async () => {
  const queryClient = new QueryClient();

  await queryClient.prefetchQuery<QueryKey>(['names'], getNames);
...
function Ninja(): ReactNode {

  const router: NextRouter = useRouter();

  const { data }: UseQueryResult = useQuery<QueryKey, Error>(['names'], getNames);
